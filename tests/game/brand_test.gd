...
func test_the_delivered_art_came_across_intact() -> void:
	assert_int(_count("res://assets/brand", ".svg")).is_equal(_count("res://docs/brand/svg/outlined", ".svg"))
	assert_int(_count("res://assets/brand", ".png")).is_equal(_count("res://docs/brand/png", ".png"))
	assert_int(_count("res://assets/brand/lockups", ".png")).is_equal(_count("res://docs/brand/png/lockups", ".png"))
...
## Every preset whose platform can carry an icon must point at the same master.
## Linux has no icon option in Godot's export settings; it takes the project icon
## at runtime, so it is deliberately not listed here.
func test_every_export_preset_that_can_carry_an_icon_uses_it() -> void:
	var text := FileAccess.get_file_as_string(PRESETS)
	for key in ["application/icon", "launcher_icons/main_192x192",
			"launcher_icons/adaptive_foreground_432x432",
			"launcher_icons/adaptive_background_432x432"]:
		assert_bool(text.contains('%s="%s"' % [key, ICON])) \
			.override_failure_message("preset key %s does not use the brand icon" % key).is_true()
...
func _count(dir_path: String, suffix: String) -> int:

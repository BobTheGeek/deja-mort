...
## Commands (added at M0; the four things CI runs)
...
`tools/run_tests.sh` passes `--ignoreHeadlessMode` because gdUnit4 blocks `--headless` by default;
nothing in `sim/` touches `Input`, so that check does not apply to us.
